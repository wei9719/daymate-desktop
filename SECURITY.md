...
CodeQL 分别扫描 TypeScript/JavaScript 与本地模型服务的 Python，高危结果与 error 级安全结果会阻止构建发布。缺失或异常的 SARIF 报告不会被视为通过；已有或被标记 suppressed 的高危结果也不豁免。本地音频只允许已支持格式，检查文件头及大小后重建为固定音频 MIME，不把任意文件类型直接作为播放地址。

可选本地模型服务只监听回环地址，免 Key、不自动加载或下载模型。它限制跨站请求、请求大小和并发，但不能认证同机其他进程，不应通过端口转发公开到网络。停止独立服务不会关闭无关模型进程；完整边界见[本地 AI 指南](docs/local-ai.md)。
